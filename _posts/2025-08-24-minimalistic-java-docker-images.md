...
I will be working on [this](https://github.com/kristi-balla/java-demo) repository. It is a simple HTMX application that prints out the current time. In addition, it exposes some metrics to prometheus. All `Dockerfiles` and configuration I reference here will be found there. If you would want to measure the following metrics on your own, I have also uploaded a convenience script.
...
This is where I'm starting at. This doesn't even build the application explicitly, but just starts off by running the server. It also defines a `HEALTHCHECK` that will be used by the benchmarking script later on.
...
One of the first things you may have noticed is that I firstly need to extract my jar to read its dependencies. Since Spring Boot produces fat JARs with a wiring that `jdeps` can not understand, I have to initially unpack it and point `jdeps` to the correct path.
...
Another interesting observation is that HTTP has to be enabled expilicitly. By default, only `file` and `resource` URL protocols are [enabled](https://www.graalvm.org/latest/reference-manual/native-image/dynamic-features/URLProtocols/).
...
Furthermore, dependencies will have to be loaded as `compileOnly`, when lazy-loading them with `runtimeOnly` would have been the "normal" recommended option. This could have unforseen consequences depending on the library.
